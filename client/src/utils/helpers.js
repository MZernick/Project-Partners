...
export function getCompatibility(user1, user2) {
    let comp = user1.compatibility;
    const result = comp.find(({ type }) => type === user2.personality);
    return result.rating;
};

export function getCompatibilityandUsername(user1, user2) {
    let comp = user1.compatibility;
    const result = comp.find(({ type }) => type === user2.personality);
...
export function indivTeamScores(teamArray, user1) {
    let tempTeamArray = [];
...
export function myTeamScore(teamArray, User1) {
...
export function oneBigTeamScore(teamArray) {
    const bigScores = [];
